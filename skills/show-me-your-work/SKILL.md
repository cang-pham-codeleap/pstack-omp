...
Use the helper `skill://show-me-your-work/scripts/log.sh <logfile> <phase> <decision> <why> <evidence> <result>` (claude code: the same file under the installed plugin, located once with `find ~/.claude -name log.sh` and reused after that). It stamps `ts`, writes the header on first use, strips stray tabs/newlines, and prefixes any cell starting with `=`, `+`, `-`, or `@` with a single quote. A bare `printf` appending a row works too, but mind those same bytes if cells come from generated or user-supplied text.
...
Before handing back, spawn a `task` subagent on `agent: "pstack-judgment"`. Self-review is not a substitute. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.

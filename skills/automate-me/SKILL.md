---
name: automate-me
description: "Use for \"automate me\", \"create/update/refresh my -mode skill\", \"turn/capture my preferences or working style into a skill\", or wanting agents to follow how the user works. Drafts or revises a personal -mode skill via manage_skill + unslop, optionally pulling fresh evidence from recent transcripts."
...
Look recursively for `.omp/skills/**/*-mode/SKILL.md`, `~/.omp/agent/skills/*-mode/SKILL.md`, and `~/.omp/agent/managed-skills/*-mode/SKILL.md` matching the user's handle. Mode skills can live in a personal category directory (`.omp/skills/<handle>/`), not only at the top level. If one exists, confirm intent with `ask` (unless they already said "update my skill" or similar):
...
Locate the active workspace's transcripts before fanning out. They live under `~/.omp/agent/sessions/<encoded-cwd>/`, where `<encoded-cwd>` is the workspace path with the home prefix dropped and every `/` replaced by `-`, prefixed with `-`. Session files are `<ISO timestamp>_<sessionId>.jsonl`, and subagent transcripts sit in a sibling `<ISO timestamp>_<sessionId>/<AgentName>.jsonl` directory. Derive the path from `PWD` rather than guessing: `d="$HOME/.omp/agent/sessions/-$(printf %s "${PWD#"$HOME"/}" | tr / -)"`. Use only that path. Don't glob across other session directories. That crosses workspace boundaries and reads private chats from unrelated projects.

Survey recent agent conversations within that scope for recurring patterns. Run a `task` batch on `agent: "pstack-judgment"`, each item taking a slice of history (e.g. last 2-4 weeks, split into 3 slices so each has enough material). Each slice mining subagent reads transcripts from the workspace-scoped path the parent provides, looks for the signals below, and returns a short structured list of patterns it saw with evidence pointers. Default signals worth hunting:
...
Apply the **unslop** skill and `manage_skill`'s writing guidelines to every line.
...
A `-mode` skill is subjective output. A `manage_skill`-style test/iterate benchmark loop isn't useful here. Vibe-check with the user: does it read like them? Did it miss anything? Then ship.

Run a description-optimization loop only if the skill's trigger accuracy turns out to be a problem in practice.

## When not to use

- User wants a task-specific skill (not working conventions): `manage_skill` alone, no mining required.

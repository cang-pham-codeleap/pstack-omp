...
Before spawning investigators, list the available MCPs in this session. Use the available-tools map when present. Otherwise inspect the configured MCP servers in `.omp/mcp.json` (project) or `~/.omp/agent/mcp.json` (user).

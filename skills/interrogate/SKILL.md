...
Launch all reviewers in a single `task` batch, one reviewer per configured panel entry, mapping positionally onto the `pstack-panel-1` through `pstack-panel-4` agents (`<n>` = the entry's 1-based index). The configured entry count sets the reviewer count, extending or shrinking the Reviewer A/B/C/D labels below to match. Otherwise use the table defaults.

| Subagent | Panel agent |
|----------|-------------|
| Reviewer A | `pstack-panel-1` |
| Reviewer B | `pstack-panel-2` |
| Reviewer C | `pstack-panel-3` |
| Reviewer D | `pstack-panel-4` |

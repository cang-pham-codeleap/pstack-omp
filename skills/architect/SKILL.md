...
Use your configured architect runners — one `pstack-panel-<n>` agent per configured panel entry (`<n>` = the entry's 1-based index), whose models come from the `pstack-panel-<n>` role (omp: `modelRoles.pstack-panel-<n>` in `~/.omp/agent/config.yml`, set by `/skill:setup-pstack`; claude code: the `model:` field in `agents/pstack-panel-<n>.md`).

...
Use your configured architect runners — one `pstack-panel-<n>` agent per configured panel entry (`<n>` = the entry's 1-based index), whose models come from the `modelRoles.pstack-panel-<n>` keys in `~/.omp/agent/config.yml`, configured by the **setup-pstack** skill (`/skill:setup-pstack`).

...
Transcripts live under `~/.omp/agent/sessions/<encoded-cwd>/`, where `<encoded-cwd>` is the workspace path with the home prefix dropped and every "/" replaced by "-", prefixed with "-" (so `~/Documents/proj` becomes `-Documents-proj`). Session files are `<ISO timestamp>_<sessionId>.jsonl`, subagent transcripts sit in the sibling `<ISO timestamp>_<sessionId>/<AgentName>.jsonl` directory, and every line is one JSON message. Derive the directory from `PWD` rather than guessing: `d="$HOME/.omp/agent/sessions/-$(printf %s "${PWD#"$HOME"/}" | tr / -)"`.

...
Transcripts live under `~/.omp/agent/sessions/<encoded-cwd>/`, where `<encoded-cwd>` is the workspace path with your home directory stripped and every "/" replaced by "-", prefixed with "-" (so `~/Documents/proj` becomes `-Documents-proj`). A workspace outside your home directory keeps its full path and is wrapped in dashes (`/private/tmp/app` becomes `--private-tmp-app--`). Derive the directory rather than guessing: `d="$HOME/.omp/agent/sessions/-$(printf '%s' "${PWD#"$HOME"/}" | tr / -)"`. Session files are `<ISO timestamp>_<sessionId>.jsonl`, subagent transcripts sit in the sibling `<ISO timestamp>_<sessionId>/<AgentName>.jsonl` directory, and every line is one JSON event (a message, a tool result, or session metadata), not necessarily one chat message. When the derived directory does not exist, the workspace sits outside your home directory, where the encoded path is wrapped in dashes: `enc=$(printf '%s' "$PWD" | tr / -); ls -d "$HOME/.omp/agent/sessions/"*"$enc-"*`. Take the exact match rather than the newest lookalike — sibling workspaces share a prefix.
...
3. Fan out across your chat history. Spawn a `task` batch on `agent: "pstack-code"`, each item taking a slice of the corpus. Tell every subagent to order candidates by real modification time (`ls -t`) and never by UUID name, grep the topic first and then read only the matching chats and only their relevant regions, and skip the current chat plus obvious noise (subagent, eval, and test chats). Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts (PRs, tickets, branches), each citing the chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.

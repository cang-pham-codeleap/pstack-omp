...
1. Capture a baseline trace on the matching surface (`browser` tool for web/Electron UIs, a `hub` PTY for CLIs and TUIs).
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a `task` subagent with `agent: "pstack-code"`, whose model comes from `modelRoles.pstack-code` in `~/.omp/agent/config.yml` (configured by the **setup-pstack** skill, `/skill:setup-pstack`). Review the diff. Capture a post-fix trace.

...
1. Capture a baseline trace on the matching surface (`browser` tool for web/Electron UIs, a `hub` PTY for CLIs and TUIs). (claude code: see `skills/poteto-mode/references/harness-surface.md` for the equivalent)
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a `task` subagent with `agent: "pstack-code"`, the `pstack-code` role (omp: `modelRoles.pstack-code` in `~/.omp/agent/config.yml`, set by `/skill:setup-pstack`; claude code: the `model:` field in `agents/pstack-code.md`). Review the diff. Capture a post-fix trace.

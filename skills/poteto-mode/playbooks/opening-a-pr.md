...
Invoked as the last step of every playbook that lands a diff. Those playbooks name it explicitly. A read-only, plan-only, or PR-lifecycle playbook does not.
...
**PRs.** Sweep the diff for slop before commit and again before review: the **no-comments** skill over comments, plus dead weight deleted per **principle-subtract-before-you-add**. Write every PR title, PR description, and commit body with **technical-writing**, then apply **unslop**. Apply every technical-writing layer except Diátaxis. Use one word for each action, keep articles, and avoid `-ing` when a plain verb works.

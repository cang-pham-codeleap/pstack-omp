...
1. Locate the prior trail. A local transcript under `~/.omp/agent/sessions/<encoded-cwd>/`, derived from `PWD` (`d="$HOME/.omp/agent/sessions/-$(printf %s "${PWD#"$HOME"/}" | tr / -)"`; subagent transcripts sit in a sibling `<ISO timestamp>_<sessionId>/` directory. Do not glob across other session directories, that crosses workspace boundaries and reads private chats from unrelated projects), a parked `task` subagent's transcript (`agent://<id>`), or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).

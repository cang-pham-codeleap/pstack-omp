...
**The agent a playbook step or a routed skill names wins.** Code delegates go to the role agent that step names. Trivial mechanical edits take `pstack-code`, the hardest changes take `pstack-judgment`. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`, `architect`, `arena`) name their own agent for diverse-model review. Respect what the step prescribes, don't override it. **Use `agent: "poteto-agent"` for an ad-hoc helper no step names**, so it reads this skill in full before working. `/skill:poteto-mode` and `poteto-agent` route through the same wrapper.
...
- **Opening a PR.** The last step of every playbook that lands a diff. Those playbooks name it explicitly. A read-only, plan-only, or PR-lifecycle playbook does not. `playbooks/opening-a-pr.md`.

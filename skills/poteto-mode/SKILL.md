...
- Any prose surface → the **unslop** skill. Your reply is a prose surface. Write it per **Writing the reply**. Agent-facing prose also follows the `manage_skill` tool's authoring rules (OMP's built-in for SKILL.md files). (claude code: see `skills/poteto-mode/references/harness-surface.md` for the equivalent)
...
- The code-review bot or the agentic security review commented → skeptical posture. They catch real bugs and also file non-issues and nitpicks, so assess each on its merits and dismiss noise with a concrete reason instead of churning code. Triage fix / dismiss / ask per `references/code-review-bot-triage.md`.
...
- **Session pickup.** Resuming or taking over a prior agent's in-flight work from a transcript, a parked subagent's artifact (`agent://<id>`), or a pushed branch. `playbooks/session-pickup.md`.

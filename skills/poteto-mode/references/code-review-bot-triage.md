...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles code-review-bot or review-automation comments. The goal is not to ignore the code-review bot by default. The goal is to stop treating every comment as a required code change.

## Decision rubric

Classify each code-review-bot thread before acting:
...
- Skip when: The PR description, screenshots, design review, or nearby code makes the visual change explicit, and the code-review-bot comment is only restating that a shared visual default changed.
...
### Upstack or stack-local usage the code-review bot cannot see

- Confidence: candidate
- Skip when: The code-review bot flags an export, component, helper, or file as unused, and the active forge's PR list and diffs, upper-stack diffs, or PR context show it is used by a later PR in the stack.
...
- Skip when: The comment body or a later code-review-bot reply explicitly says the finding is withdrawn, compliant, or a false positive, and the agent can verify the relevant rule locally.
...
- Source: one sticky-occlusion PR: six code-review-bot passes, roughly eighteen findings, every one fixed rather than dismissed.
...
- Skip when: Never skip the verification itself; it costs one command. When a PR
  ...
  a SKILL.md, snapshot of doc wording), and the code-review bot claims "the test no longer
...
- Source: one prose-pinning PR with eight code-review-bot passes; the claim was real on

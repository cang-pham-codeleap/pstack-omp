...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) sweeps the diff for slop before each commit and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. The sweep is pstack's own, and nothing external runs it: the [`/no-comments`](../../skills/no-comments/SKILL.md) pass over the comments, plus dead weight deleted per the subtract-before-you-add principle. You can ask for the same outcome in plain words: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
...
The division of labor is worth keeping straight. The slop sweep cleans the code, `/unslop` cleans the prose, and `/no-comments` hands the comments to a reviewer who didn't write them.

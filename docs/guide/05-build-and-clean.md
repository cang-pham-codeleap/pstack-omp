...
```text
/skill:poteto-mode this command emits two records after a retry. repro first, then fix and verify.
```

A feature prompt states the behavior and what must not change:

```text
/skill:poteto-mode add a --json flag. text output stays byte-identical. verify both forms.
```

A refactoring prompt pins behavior before structure moves:

```text
/skill:poteto-mode move parsing into one module, zero behavior change. record the current output first and prove it's unchanged after.
```

A perf prompt states the measurement, not a vibe:

```text
/skill:poteto-mode startup takes 1.8s on this fixture. trace it, fix the measured cause, show me before and after.
```
...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) sweeps the diff for slop before each commit and applies [`/skill:unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. The sweep is pstack's own, and nothing external runs it: the [`/skill:no-comments`](../../skills/no-comments/SKILL.md) pass over the comments, plus dead weight deleted per the subtract-before-you-add principle. You can ask for the same outcome in plain words: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
...
The division of labor is worth keeping straight. The slop sweep cleans the code, `/skill:unslop` cleans the prose, and `/skill:no-comments` hands the comments to a reviewer who didn't write them.

**Pitfall:** cleanup is not optional polish. A diff with narrating comments and defensive dead weight reads as unfinished to reviewers, and the extra code is where the next bug hides. If the diff feels padded, say `sweep the diff for slop` before you commit, not after review calls it out.
